...
def fast_rcnn_inference_single_image(boxes, scores, image_shape, score_thresh, nms_thresh, topk_per_image):
    ...
    # Convert to Boxex to use the 'clip' function ...
    boxes = Boxes(boxes.reshape(-1, 4))
    boxes.clip(image_shape)
    boxes = boxes.tensor.view(-1, num_bbox_reg_classes, 4) # R x C x 4

    # Filter results based on detection scores
    filter_mask = scores > score_thresh # RxK
    # R' x 2, Firt column contains indices of the R predictions;
    # Second column contians indices of classes.
...
class FastRCNNOutputs(object):
    ...
    It provides methods htat are used to decode the outputs of a Fast RCNN head
    """
    def __init__(
        self,
        box2box_transform,
        ...
        if len(proposals):
            ...
            ), "Proposal should not require gradients"
