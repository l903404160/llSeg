...
class BoxMode(IntEnum):
    ...
    def convert(box: _RawBoxType, from_mode:"BoxMode", to_mode:"BoxMode") -> _RawBoxType:
        ...
        assert to_mode.value not in [
            BoxMode.XYXY_REL, BoxMode.XYWH_REL
...
class Boxes:
    ...
    def clip(self, box_size: Tuple[int, int]):
        ...
        """
        assert torch.isfinite(self.tensor).all(), " Box tnesor contains infinite or NaN"
        h, w = box_size
    ...
    def nonempty(self, threshold: float=0.0) -> torch.Tensor:
        ...
        A box is considered empty, if eigher of its side is no larger than threshold
        Args:
            threshold:
    ...
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]):
        ...
        b = self.tensor[item]
        assert b.dim() == 2, "Indesing on Boxes with {} failed to return a matrix!".format(item)
        return Boxes(b)
    ...
    def inside_box(self, box_size:Tuple[int, int], boundary_threshold: int=0) -> torch.Tensor:
        """
        Args:
            box_size: (h,w) Size of the reference box
            boundary_threshold: (int) Boxes that extend beyond the reference box boundary by more that boundary_threshold are considered "outside".
    ...
    def cat(cls, boxes_list):
        """
        Concatenates a list of Boses into a single Boxes
        Args:
            boxes_list:
...
def matched_boxlist_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    ...
    Returns:
        (tensor) iou, sized [N].
    """
    assert len(boxes1) == len(boxes2), "boxlists should have the sane number of entries ," \
