...
from utils.nn.precise_bn import get_bn_modules
...
__all__ = ['default_argument_setup', 'default_setup',
           'DefaultPredictor', 'DefaultTrainer']

def default_argument_setup():
    """
    Create a parser with some common arguments used by the current
    :return:
    """
    ...
    port = 2 ** int(random.randint(9, 15)) + 2 ** 14 + hash(os.getuid() if sys.platform != "win32" else 1) % 2 ** 14
...
def default_setup(cfg, args):
    """
    perform some basic common setups at the begining of a job, including:
        1. Setup the root logger
    ...
    rank = comm.get_rank()
    # setup_logger(output=output_dir, distributed_rank=rank, name="OUCWheel")
    logger = setup_logger(output_dir, distributed_rank=rank)

    logger.info("Rank of current process : {}. Wrold size: {}".format(rank, comm.get_world_size()))
...
class DefaultPredictor:
    ...
    If you'd like to do anything more fancy, please refer to its source code as exampples to build and use the model manually
    Attributes:
...
class DefaultTrainer(SimpleTrainer):
    """
    A trainer with default training logic. COmpared to 'SimpleTrainer', it contains the
    following logic in addition

    1. Create model, optimizer, scheduler, dataloader from the given config
    2. Load a checkpoint or 'cfg.MODEL.WEIGHTS', if exists, when 'resume_or_load' is called
    3. Register a few common hooks

    It is created to simplify the **standard model training workflow** and reduce code boilerplate
    for users who only need the standard training workflow, with standard features.
    It means this class makes *many assumptions* about your training logic that
    may easily become invalid in a new research. In fact, any assmptions beyond those
    made in the "class"'SimpleTrainer' are too much for research

    The code of this class has been annotated about restrictive assumptions it mades.
    When they do not work for you, you're encouraged to:

    1. Overwrite methods of this class, OR:
    2. Use :class:'SimpleTrainer', which only does minimal SGD training and nothing else.
        You can then add your own hooks if needed. Or:
    3. Write your own training loop similar to 'tools/plain_train_net.py'

    Also note that the behavior of this class, like other fucntions/classes in this file,
    is not stable, since it is meant to represent the "common default behavior"
    It is only guranteed to work well with the standard models and training workflow in D2
    ...
    def __init__(self, cfg):
        ...
        :param cfg:
        """
        logger = logging.getLogger('OUCWheel')
    ...
    def resume_or_load(self, resume=True):
        ...
        if self.start_iter != 0:
            ...
            # Tips: When remake a model into the target format, we set the iteration to 0
            # Hence when start_iter == 0 it indicates we do not perform resue
            # and the :start_iter should not be add by 1
            self.start_iter += 1
