...
class Convertor(object):
    ...
    def load_drones(self):
        ...
        for split in self.splits:
            if 'train' in split:
                img_path = osp.join(self.root_dir, 'train', split, 'images')
            else:
                img_path = osp.join(self.root_dir, 'test', split, 'images')
    ...
    def drones2coco(self):
        ...
        for split in self.splits:
            ...
            for name in names:
                if 'train' in split:
                    width, height = imagesize.get(osp.join(self.root_dir, 'train', split, 'images', '{}.jpg'.format(name)))
                else:
                    width, height = imagesize.get(osp.join(self.root_dir, 'test', split, 'images', '{}.jpg'.format(name)))
                image = {
                    "license": 3,
                ...
                if split is not 'test':
                    if 'train' in split:
                        with open(osp.join(self.root_dir, 'train', split, 'annotations', '{}.txt'.format(name)), 'r') as reader:
                            annos = reader.readlines()
                    else:
                        # with open(osp.join(self.root_dir, 'val', split, 'annotations', '{}.txt'.format(name)), 'r') as reader:
                        with open('/home/haida_sunxin/lqx/code/github/RRNet_train/test.txt',
                                  'r') as reader:
                            annos = reader.readlines()
                    for anno in annos:
...
if __name__ == '__main__':
    convertor = Convertor('/home/haida_sunxin/lqx/data/DronesDET/', '/home/haida_sunxin/lqx/data/DronesDET')
    convertor.start()