...
class RepeatFactorTrainingSampler(Sampler):
    ...
    def __init__(self, dataset_dicts, repeat_thresh, shuffle=True, seed=None):
        """
        :param dataset_dicts: annotations
        :param repeat_thresh:
        :param shuffle:
        :param seed:
        """
        ...
        # Get fractional repeat factors and split into whole number
    ...
    def _get_epoch_indices(self, generator):
        """
        :param generator: torch.Generator: pseudo random number generator used for stochastic rounding.
        :return: torch.Tensor: list of dataset indices to use in one epoch. Each index is repeated based on its
        calculated repeat factor
        """
    ...
    def _infinite_indices(self):
        ...
        g.manual_seed(self._seed)
        while True:
            # Sample indices with repeats determined by stochastic rounding;
            # each "epoch" may have a slightly different size due to the rounding.
            indices = self._get_epoch_indices(g)
            if self._shuffle:
                randperm = torch.randnperm(len(indices), generator=g)
